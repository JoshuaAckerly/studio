...
class Player extends GameObject {
    ...
    createPlayerDeferred() {
        ...
        try {
            ...
            // Make sprite visible and properly sized
            this.sprite.setOrigin(0.5, 1);
            this.sprite.setScale(3.0); // Use a larger, more visible scale
            this.sprite.setVisible(true);
            this.sprite.setAlpha(1);
            console.log('[Player] Created physics sprite at', this.x, this.y, 'with scale 3.0');
            try {
            ...
            console.warn('[Player] Physics sprite creation failed:', e.message);
            ...
            try {
                ...
                this.sprite.setVisible(true);
                this.sprite.setAlpha(1);
                console.log('[Player] Created fallback image at', this.x, this.y, 'with scale 3.0');
            } catch (e2) {
                console.error('[Player] Both physics and image sprite creation failed:', e2.message);
            }
        }
    ...
    // Helper to attempt creating the Spine display with retries. Some builds
        ...
        const finalizeSpineVisual = () => {
            try {
                ...
                // Apply target pixel height if configured
                try {
                    if (GameConfig && GameConfig.player && GameConfig.player.targetPixelHeight) {
                        this.setDisplayHeight(GameConfig.player.targetPixelHeight);
                    }
                ...
                // Normalize sign of scaleX to match flip state
                try {
                    const baseScale = this.getAppliedScale();
                    if (this.sprite && this.sprite.flipX) {
                        this.spine.scaleX = -Math.abs(baseScale);
                    } else {
                        this.spine.scaleX = Math.abs(baseScale);
                    }
                    this.spine.scaleY = Math.abs(baseScale);
                } catch (e) {
                    // ignore
                }
        ...
        setTimeout(() => {
            if (!this.spine && this.scene && this.scene.add && typeof this.scene.add.spine === 'function') {
                try {
                    ...
                    if (spine) {
                        this.spine = spine;
                        // Use proper scale from config instead of hardcoded 0.6
                        const baseScale = this.getAppliedScale();
                        spine.setScale(baseScale);
    ...
    getAppliedScale() {
        try {
            if (typeof this._overrideScale === 'number' && !Number.isNaN(this._overrideScale) && isFinite(this._overrideScale)) {
                const scale = Math.abs(this._overrideScale);
                console.log('[Player] Using override scale:', scale);
                return scale;
            }
            const cfg = (GameConfig && GameConfig.player && typeof GameConfig.player.scale === 'number') ? GameConfig.player.scale : 1;
            const scale = Math.abs(cfg || 1);
            console.log('[Player] Using config scale:', scale);
            return scale;
        } catch (e) {
            console.log('[Player] Using fallback scale: 1');
            return 1;
        }
    }
    ...
    setDisplayHeight(targetPx) {
        try {
            ...
            try { console.info('[Player] setDisplayHeight called targetPx=', targetPx, 'current overrideScale=', this._overrideScale); } catch (e) {}
            ...
            const deriveFromTextureKey = (key) => {
                try {
                    ...
                    if (!this.scene.textures.exists(key)) return null;
                        ...
                        if (!tex) return null;
                        try { console.info('[Player] deriveFromTextureKey checking key=', key); } catch (e) {}
                    ...
                    try {
                        ...
                        if (frameNames && frameNames.length) {
                            ...
                            if (frame && typeof frame.height === 'number' && frame.height > 0) {
                                try { console.info('[Player] deriveFromTextureKey: found frame', fn, 'height=', frame.height); } catch (e) {}
                                return frame.height;
                            }
                        }
                    } catch (e) {}
                    ...
                    try {
                        ...
                        if (src && typeof src.height === 'number' && src.height > 0) {
                            try { console.info('[Player] deriveFromTextureKey: source image height for', key, '=', src.height, ' (naturalHeight=', src.naturalHeight || 'n/a', 'width=', src.width || 'n/a') } catch (e) {}
                            return src.height;
                        }
                    try { console.info('[Player] deriveFromTextureKey: no usable height found for key=', key); } catch (e) {}
                    } catch (e) {}
                } catch (e) {}
                return null;
            };
            ...
            let sourceH = null;
            const candidatesDebug = { persistentFallbackSprite: null, persistentFallbackImage: null, spine: null, sprite: null };
            try {
                if (this._persistentFallbackSprite && this._persistentFallbackSprite.texture) {
                    const k = this._persistentFallbackSprite.texture.key;
                    candidatesDebug.persistentFallbackSprite = k;
                    sourceH = deriveFromTextureKey(k) || sourceH;
                }
                if (!sourceH && this._persistentFallbackImage && this._persistentFallbackImage.texture) {
                    const k = this._persistentFallbackImage.texture.key;
                    candidatesDebug.persistentFallbackImage = k;
                    sourceH = deriveFromTextureKey(k) || sourceH;
                }
                if (!sourceH && this.spine) {
                    ...
                    try {
                        ...
                        try { candidatesDebug.spine = { displayHeight: disp, scale: curScale }; } catch (e) {}
                    } catch (e) {}
                }
                if (!sourceH && this.sprite && this.sprite.texture) {
                    const k = this.sprite.texture.key;
                    candidatesDebug.sprite = k;
                    sourceH = deriveFromTextureKey(k) || sourceH;
                }
            } catch (e) {}

            try { console.info('[Player] setDisplayHeight: candidate keys/values=', candidatesDebug, 'resolved sourceH=', sourceH); } catch (e) {}

            if (!sourceH || sourceH <= 0) return false;

            // Guard against implausibly-small source heights (e.g. 1px) which
            // would produce huge scales. Instead of aborting immediately, attempt
            // to recover using more reliable metadata (skeleton JSON/cache) and
            // finally fall back to a conservative assumed source height so the
            // player remains visible. Log concise warnings for debugging.
            if (sourceH < 4) {
                try {
                    // Try to read skeleton data cached by AssetManager as a best-effort
                    // source for the native visual height. AssetManager.populate puts
                    // parsed skeleton under scene.cache.custom['spine-skeleton-data']
                    // or the original JSON under cache.json ('noteleks-skeleton-data').
                    let sd = null;
                    try {
                        if (this.scene && this.scene.cache && this.scene.cache.custom && this.scene.cache.custom['spine-skeleton-data']) sd = this.scene.cache.custom['spine-skeleton-data'];
                    } catch (e) { sd = null; }
                    try {
                        if (!sd && this.scene && this.scene.cache && this.scene.cache.json && typeof this.scene.cache.json.get === 'function') {
                            sd = this.scene.cache.json.get('noteleks-skeleton-data') || this.scene.cache.json.get('noteleks-data') || sd;
                        }
                    } catch (e) { /* ignore */ }
                    try {
                        if (!sd && typeof window !== 'undefined' && window.NOTELEKS_DIAG && window.NOTELEKS_DIAG.skeleton) sd = window.NOTELEKS_DIAG.skeleton;
                    } catch (e) {}

                    let estimated = null;
                    try {
                        if (sd && typeof sd === 'object') {
                            // Common shapes: raw skeleton object or nested under `skeleton` key
                            if (typeof sd.height === 'number' && sd.height > 0) estimated = sd.height;
                            else if (sd.skeleton && typeof sd.skeleton.height === 'number') estimated = sd.skeleton.height;
                            else if (sd.spine && typeof sd.spine.height === 'number') estimated = sd.spine.height;
                        }
                    } catch (e) { /* ignore */ }

                    if (estimated && estimated >= 4) {
                        sourceH = Math.round(estimated);
                        console.info('[Player] setDisplayHeight: used skeleton-data height fallback', sourceH);
                    }
                } catch (e) {
                    // ignore recovery errors
                }

                // If recovery didn't produce a usable source height, fall back to a
                // conservative assumed height so we at least apply a sane scale instead
                // of skipping the operation entirely. This avoids invisible players when
                // Spine internals report tiny values (for example when a global scale
                // of 0.1 is applied earlier).
                if (!sourceH || sourceH < 4) {
                    const conservative = (GameConfig && GameConfig.player && typeof GameConfig.player.defaultSourceHeight === 'number') ? GameConfig.player.defaultSourceHeight : 48;
                    try { console.warn('[Player] setDisplayHeight: source height too small (', sourceH, ') — using conservative fallback', conservative); } catch (e) {}
                    sourceH = conservative;
                }
            }
    ...
    if (!this.spine && !this._persistentFallbackSprite && !this._persistentFallbackImage && !overlayEnabled) return;
        ...
        try {
            ...
            // Ensure proper scale is maintained
            try {
                const baseScale = this.getAppliedScale();
                if (typeof this.spine.setScale === 'function') {
                    const sign = (this.sprite && this.sprite.flipX) ? -1 : 1;
                    this.spine.setScale(sign * baseScale, baseScale);
                }
            } catch (e) {
                // ignore scale errors
            }
