...
class TouchInputComponent extends Component {
    ...
    createJumpButton() {
        ...
        const buttonY = this.jumpButtonY || this.scene.scale.height * 0.7;
    ...
    createAttackButton() {
        const buttonX = this.scene.scale.width - 120;
        const buttonY = this.attackButtonY || this.scene.scale.height * 0.8;
    ...
    handleTouchStart(pointer) {
        ...
        // Check joystick area
        const joystickDistance = Phaser.Math.Distance.Between(touchX, touchY, this.joystickCenter.x, this.joystickCenter.y);
        ...
        if (
            touchX > this.scene.scale.width / 2 &&
     ...
     */
    highlightButton(_button, _color) {
